...
public class CommonService {
	...
	/**
	 * “行业名企”模块：“选择行业”下拉出来的行业名称
	 * 在t_show_homeareas表中type=3,father=0, status=0，结果按照rank升序和update_date降序排列 的name
	 */
	public List<CommonItem> queryHomeAreasForSelect() {
		// 设置查询条件
		TShowHomeareas search = new TShowHomeareas();
		search.setFather(0L);
		search.setType(3);
		search.setStatus(0);
		// 查询1级“热门行业”
		List<TShowHomeareas> showHomeareas = showHomeareasDao.searchByCriteria(1, 999, search);
		List<CommonItem> result = new ArrayList<CommonItem>();
		for (TShowHomeareas homearea : showHomeareas) {
			CommonItem rootItem = new CommonItem();
			rootItem.setId(homearea.getTheId().toString());  //存入CommonItem的id应该是表中的the_id字段值
